...
import com.eneserkocak.gezirehberi.databinding.FragmentBathMapsBinding
import com.eneserkocak.gezirehberi.databinding.FragmentChurchMapsBinding
...
class ChurchMapsFragment : BaseFragment<FragmentChurchMapsBinding>(R.layout.fragment_church_maps),
    OnMapReadyCallback {
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        viewModel.selectedFavoriteChurch.observe(viewLifecycleOwner) { church->
            church?.let {
                mMap.addMarker(MarkerOptions().position(LatLng(it.favoriteChurchLat,it.favoriteChurchLong)).title(it.favoriteChurchName)                    )
                mMap.moveCamera(
                    CameraUpdateFactory.newLatLngZoom(
                        LatLng(it.favoriteChurchLat,it.favoriteChurchLong), 12f))
            }
        }
