...
import com.eneserkocak.gezirehberi.databinding.FragmentLakeMapsBinding
...
class LakeMapsFragment : BaseFragment<FragmentLakeMapsBinding>(R.layout.fragment_lake_maps),
    OnMapReadyCallback {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        sharedPreferences = requireActivity().getSharedPreferences("com.eneserkocaks",AppCompatActivity.MODE_PRIVATE)
        trackBoolean = false
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        viewModel.selectedFavoriteLake.observe(viewLifecycleOwner) {
            it?.let {
                mMap.addMarker(MarkerOptions().position(LatLng(it.favoriteLakeLat,it.favoriteLakeLong)).title(it.favoriteLakeName)                    )
                mMap.moveCamera(
                    CameraUpdateFactory.newLatLngZoom(
                        LatLng(it.favoriteLakeLat,it.favoriteLakeLong), 12f))
            }
        }
