...
import com.eneserkocak.gezirehberi.databinding.FragmentAncientMapsBinding
...
class AncientMapsFragment : BaseFragment<FragmentAncientMapsBinding>(R.layout.fragment_ancient_maps),OnMapReadyCallback {
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        viewModel.selectedFavoriteAnciencity.observe(viewLifecycleOwner) { mosque->
            mosque?.let {
                mMap.addMarker(MarkerOptions().position(LatLng(it.favoriteAncientcityLat,it.favoriteAncientcityLong)).title(it.favoriteAncientcityName)                    )
                mMap.moveCamera(
                    CameraUpdateFactory.newLatLngZoom(
                        LatLng(it.favoriteAncientcityLat,it.favoriteAncientcityLong), 12f))
            }
        }
