...
class MosqueMapsFragment : BaseFragment<FragmentMosqueMapsBinding>(R.layout.fragment_mosque_maps),OnMapReadyCallback {
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        viewModel.selectedFavoriteMosque.observe(viewLifecycleOwner) { mosque->
            mosque?.let {
                mMap.addMarker(MarkerOptions().position(LatLng(it.favoriteMosqueLat,it.favoriteMosqueLong)).title(it.favoriteMosqueName)                    )
                mMap.moveCamera(
                    CameraUpdateFactory.newLatLngZoom(
                        LatLng(it.favoriteMosqueLat,it.favoriteMosqueLong), 12f))
            }
        }
