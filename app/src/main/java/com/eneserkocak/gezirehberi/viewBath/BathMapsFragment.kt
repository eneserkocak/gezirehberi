...
import com.eneserkocak.gezirehberi.databinding.FragmentMosqueMapsBinding
...
class BathMapsFragment : BaseFragment<FragmentBathMapsBinding>(R.layout.fragment_bath_maps),
    OnMapReadyCallback {
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        viewModel.selectedFavoriteBath.observe(viewLifecycleOwner) { bath->
            bath?.let {
                mMap.addMarker(MarkerOptions().position(LatLng(it.favoriteBathLat,it.favoriteBathLong)).title(it.favoriteBathName)                    )
                mMap.moveCamera(
                    CameraUpdateFactory.newLatLngZoom(
                        LatLng(it.favoriteBathLat,it.favoriteBathLong), 12f))
            }
        }
