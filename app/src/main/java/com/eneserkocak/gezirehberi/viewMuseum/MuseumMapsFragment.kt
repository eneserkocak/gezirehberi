...
import com.eneserkocak.gezirehberi.databinding.FragmentMuseumMapsBinding
...
class MuseumMapsFragment : BaseFragment<FragmentMuseumMapsBinding>(R.layout.fragment_museum_maps),OnMapReadyCallback {
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        viewModel.selectedFavoriteMuseum.observe(viewLifecycleOwner) { museum->
            museum?.let {
                mMap.addMarker(MarkerOptions().position(LatLng(it.favoriteMuseumLat,it.favoriteMuseumLong)).title(it.favoriteMuseumName)                    )
                mMap.moveCamera(
                    CameraUpdateFactory.newLatLngZoom(
                        LatLng(it.favoriteMuseumLat,it.favoriteMuseumLong), 12f))
            }
        }
