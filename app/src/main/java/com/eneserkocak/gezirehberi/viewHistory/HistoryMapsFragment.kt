...
import com.eneserkocak.gezirehberi.databinding.FragmentHistoryMapsBinding
...
class HistoryMapsFragment : BaseFragment<FragmentHistoryMapsBinding>(R.layout.fragment_history_maps), OnMapReadyCallback {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val mapFragment = childFragmentManager.findFragmentById(R.id.mapsHistory) as SupportMapFragment?
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        viewModel.selectedFavoriteHistory.observe(viewLifecycleOwner) { historical->
            historical?.let {
                mMap.addMarker(MarkerOptions().position(LatLng(it.favoriteHistoryLat,it.favoriteHistoryLong)).title(it.favoriteHistoryName)                    )
                mMap.moveCamera(
                    CameraUpdateFactory.newLatLngZoom(
                        LatLng(it.favoriteHistoryLat,it.favoriteHistoryLong), 12f))
            }
        }
