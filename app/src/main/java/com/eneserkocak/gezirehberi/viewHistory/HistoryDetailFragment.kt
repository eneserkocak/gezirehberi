...
import com.eneserkocak.gezirehberi.databinding.FragmentHistoryDetailBinding
...
class HistoryDetailFragment : BaseFragment<FragmentHistoryDetailBinding>(R.layout.fragment_history_detail), OnMapReadyCallback {
    ...
    lateinit var selectededHistory: History
    ...
    fun observeLiveData(){
        viewModel.selectedHistory.observe(viewLifecycleOwner){ lake->

            lake?.let {

                selectededHistory= it
                binding.historyName.text= it.placeName
                binding.historyExplanation.text= it.explanation
                AppUtil.pictureHistory(it,requireContext(),binding.historyDetailImage)
            }
    ...
    fun addFavoriteDialog(){
        ...
        alert.setPositiveButton("EVET", object : DialogInterface.OnClickListener{
            override fun onClick(dialog: DialogInterface?, which: Int) {
                ...
                val id = selectededHistory.id
                val name= selectededHistory.placeName
                val location= selectededHistory.district
                val lat = selectededHistory.lat
                val long= selectededHistory.long


                val favoriteHistorical= FavoriteHistory(id,name,location,lat,long)

                viewModel.selectedFavoriteHistory.value= favoriteHistorical
                FavoriteHistoryDatabase.getInstance(requireContext())?.favoriteHistoryDao()?.insertAll(favoriteHistorical)
