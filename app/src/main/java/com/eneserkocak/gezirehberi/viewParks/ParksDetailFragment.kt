...
import com.eneserkocak.gezirehberi.databinding.FragmentParksDetailBinding
...
class ParksDetailFragment() : BaseFragment<FragmentParksDetailBinding>(R.layout.fragment_parks_detail), OnMapReadyCallback {
    ...
    private var parkLatitude: Double? = null
    private var parkLongitude: Double? = null

    lateinit var selectededPark: Park

   /* val lat:Double=37.886053
    val long:Double=32.426158*/
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.selectedPark.observe(viewLifecycleOwner){
            it?.let { park->
        ...
        viewModel.selectedPark.observe(viewLifecycleOwner){ park->
            ...
            park?.let {
                ...
                binding.parkAdi.text=it.yerAdi
                binding.parkAciklama.text = it.aciklama
                AppUtil.gorselIndir(it,requireContext(),binding.parkDetailImage)
                }
    ...
    fun addFavoriteDialog(){
        ...
        alert.setPositiveButton("EVET", object : DialogInterface.OnClickListener{
            override fun onClick(dialog: DialogInterface?, which: Int) {
                ...
                val id = selectededPark.id
                val name= selectededPark.yerAdi
                val location= selectededPark.ilce
                val lat = selectededPark.lat
                val long= selectededPark.long


                val favoriteParks= FavoritePark(id,name,location,lat,long)

                viewModel.selectedFavoritePark.value=favoriteParks
                FavoriteParksDatabase.getInstance(requireContext())?.favoriteParksDao()?.insertAll(favoriteParks)
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        mMap = googleMap
            ...
            viewModel.selectedPark.observe(viewLifecycleOwner) {
                it?.let {
                    mMap.addMarker(MarkerOptions().position(LatLng(it.lat,it.long)).title(it.yerAdi)                    )
                    mMap.moveCamera(
